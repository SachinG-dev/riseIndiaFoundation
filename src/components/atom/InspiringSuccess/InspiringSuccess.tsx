...
export function InspiringSuccess({
  title,
  sectionClasses,
  inspiringSuccessCardData,
}: InspiringSuccessProps) {
  return (
        ...
        <div className="row">
          <div className="col">
            {inspiringSuccessCardData.map((data, index) => {
              return data.cardAlignment === "Left" ? (
                <div
                  className="inspiring-success__box d-flex ms-auto gap-4 flex-column-reverse flex-sm-row mt-5 align-items-start"
                  key={index}
                >
                  <div className="inspiring-success__box--content text-center text-sm-end">
                    {data?.cardTitle && (
                      <h3 className="text-primary mb-0">{data.cardTitle}</h3>
                    )}
                    {data?.cardDesc && <p className="mb-0">{data.cardDesc}</p>}
                  </div>
                  <img
                    className="img-fluid inspiring-success__box--image"
                    src={data?.imageURL}
                    alt=""
                  />
                </div>
                ...
                <div
                  className="inspiring-success__box d-flex flex-column flex-sm-row me-auto gap-4 mt-5"
                  key={index}
                >
                  <img
                    className="img-fluid inspiring-success__box--image"
