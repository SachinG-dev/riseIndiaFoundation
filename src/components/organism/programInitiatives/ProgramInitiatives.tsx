        ...
        <div className="row flex-nowrap flex-lg-wrap justify-content-lg-center overflow-scroll hide-scrollbar">
          {cardData.map((data, index) => {
            return (
              <div
                key={index}
                className={`col-11 col-sm-5 col-lg-4 text-center mt-4 ${overAllClasses.cardClasses}`}
              >
                <div className="program-initiatives__box bg-white p-4 h-100">
                  <img
                    src={data.iconSrc}
                    alt="program-initiatives-icon"
                    className="program-initiatives__icon"
                  />
                  <h3 className={`pt-4 ${overAllClasses.cardTitleClass}`}>
                    {data.cardTitle}
                  </h3>
                  ...
                  <div className="d-flex justify-content-center">
                    {cardButtonText && (
                      <Button
                        onClick={() => onChangeCallback(data.buttonURL)}
                        variant="primaryArrow"
                        className={`${overAllClasses.cardButtonClass}`}
                      >
                        {cardButtonText}
                      </Button>
                    )}
